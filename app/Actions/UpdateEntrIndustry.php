...
/**
 * Industries where the entrepreneur would like to work in
 * and industries where the entrepreneur has worked
 * Class UpdateEntrPfdIndustry
 * @package App\Actions
 */
class UpdateEntrIndustry
{
    /**
    ...
    public static function execute(EntrepreneurProfile $profile, array $data)
    {
        ...
        if (count($data)) {
            DB::transaction(function () use ($data, $profile) {
                $profile->expIndustries()->delete();
                $profile->expIndustries()->createMany($data);
            });
        }
    }
}

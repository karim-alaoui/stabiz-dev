...
return [
     ...
     */
    'title' => 'Stabiz API Documentation',
...
With each api call make sure to include this in the header

**`"Accept": "application/json"`**

<aside class="notice">Response Language</aside>
Currently the application supports two languages English (**`en`**), Japanese (**`ja`**).
You can request in the header which language you want the response in by providing this in the header <br>
**`"lang": "en"`** <br>
On the staging and dev server, response will be in English by default and on the production, it will
be in Japanese. However, you can change the language anytime using the **`lang`** header.

**
Sometimes, you might still see English text even after sending the **`lang`** as **`ja`**,
it's not an error. It's because the translation is not ready yet.
**

<aside class="notice">
Some notes on Response formats and HTTP status codes
</aside>

**All the **`400`** and **`500`** series code will have **`message`** key in response metadata**

### Validation Error - 422
Any time if the validation fails, it would return **`422`** HTTP status code.
...
```json
{
    ...
    "errors": {
        ...
        "email": [
            ...
            "The email field has to be less than 30 characters"
        ]
    }
}
...
Anytime the user try to access which the user doesn't have permission to access, it would return **`403`**

### Unauthenticated
The error code **`401`** means that route has authentication protected route and
the auth token that you sent is either expired or not valid.

### Too many requests
API has rate limit to mitigate DDOS attack. When the endpoint hits the limit, it returns **`429`**

### Other error
Any other error code will be **`400`** Sometimes it may return the some extra info (see **`500`**)
on the staging and dev server for debugging purpose.

### Internal Error - 500
**If you see any **`500`** error code, this is because of some error on the backend, inform backend dev in that case.
If you're on staging or dev server, along with **`message`** key, other keys like <br>
**`actual_error`**, **`file`**, **`line_no`** <br>
will be returned. You can pass those information to the dev as it's returned for debugging purposes on the
